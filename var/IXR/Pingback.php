...
class Pingback
{
    ...
    {
        /** Loại bỏ thẻ html, chỉ để lại <a> */
        $text = Common::stripTags($this->html, '<a href="">');

        /** Ở đây, báo giá $target được dành riêng để sử dụng sau này. */
        $pregLink = preg_quote($this->target);

        /** Tìm dòng dài nhất chứa liên kết đích là $finalTex t*/
        $finalText = null;
        $lines = explode("\n", $text);
